...
func NewProvider(ctx *pulumi.Context,
	...
	if isZero(args.AccountId) {
		args.AccountId = pulumi.StringPtr(getEnvOrDefault("", nil, "DATABRICKS_ACCOUNT_ID").(string))
	}
	if isZero(args.ConfigFile) {
		args.ConfigFile = pulumi.StringPtr(getEnvOrDefault("", nil, "DATABRICKS_CONFIG_FILE").(string))
	}
	if isZero(args.DebugHeaders) {
		args.DebugHeaders = pulumi.BoolPtr(getEnvOrDefault(false, parseEnvBool, "DATABRICKS_DEBUG_HEADERS").(bool))
	}
	if isZero(args.DebugTruncateBytes) {
		args.DebugTruncateBytes = pulumi.IntPtr(getEnvOrDefault(0, parseEnvInt, "DATABRICKS_DEBUG_TRUNCATE_BYTES").(int))
	}
	if isZero(args.Host) {
		args.Host = pulumi.StringPtr(getEnvOrDefault("", nil, "DATABRICKS_HOST").(string))
	}
	if isZero(args.Password) {
		args.Password = pulumi.StringPtr(getEnvOrDefault("", nil, "DATABRICKS_PASSWORD").(string))
	}
	if isZero(args.Profile) {
		args.Profile = pulumi.StringPtr(getEnvOrDefault("", nil, "DATABRICKS_PROFILE").(string))
	}
	if isZero(args.RateLimit) {
		args.RateLimit = pulumi.IntPtr(getEnvOrDefault(0, parseEnvInt, "DATABRICKS_RATE_LIMIT").(int))
	}
	if isZero(args.Token) {
		args.Token = pulumi.StringPtr(getEnvOrDefault("", nil, "DATABRICKS_TOKEN").(string))
	}
	if isZero(args.Username) {
		args.Username = pulumi.StringPtr(getEnvOrDefault("", nil, "DATABRICKS_USERNAME").(string))
	}
